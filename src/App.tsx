...
export default function App() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#/', '').replace('#', '') as PageType;
      if (hash && ['home', 'tafsir', 'fiqh', 'hadiths', 'burdah', 'prophetes', 'lecture_coran', 'apropos', 'quiz', 'legal'].includes(hash)) {
        setCurrentPage(hash);
      } else {
        setCurrentPage('home');
      }
    ...
    switch (currentPage) {
      ...
      default:
        if (playlists[currentPage]) {
          return <VideoSection info={playlists[currentPage]} setPage={setPage} />;
        }
        return <Home setPage={setPage} />;
    }
  };
