...
import { BookOpen, PlayCircle, Heart, Star, HelpCircle, Info, MessageCircle, BookMarked } from 'lucide-react';
...
export function Home({ setPage }: HomeProps) {
  ...
  const cards = [
    ...
    { id: 'hadiths', title: 'Hadiths', icon: MessageCircle, desc: 'Paroles du Prophète ﷺ et leçons de sagesse.' },
    { id: 'burdah', title: 'Spiritualité (Burdah)', icon: Heart, desc: 'Chants de louanges et éloges sur le Prophète ﷺ.' },
    { id: 'apropos', title: 'À propos', icon: Info, desc: 'Notre mission et notre équipe.' },
  ];
        ...
        {cards.map((card, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`group cursor-pointer bg-daara-surface p-8 rounded-2xl shadow-lg border transition-all duration-500 hover:-translate-y-2 relative overflow-hidden border-daara-gold/10 hover:border-daara-gold/40`}
            >
              ...
              <div className={`w-14 h-14 rounded-xl bg-daara-bg/50 border border-daara-gold/20 flex items-center justify-center mb-6 transition-all duration-500 relative z-10 group-hover:bg-daara-gold group-hover:border-daara-gold text-daara-gold group-hover:text-daara-bg`}>
                <Icon className="w-7 h-7" />
              </div>
              <h3 className={`text-2xl font-serif font-bold mb-3 transition-colors relative z-10 text-daara-text group-hover:text-daara-gold-light`}>
                {card.title}
              </h3>
